    ...
    for i in range(len(board)):
        for j in range(len(board[0])):
            if j < len(board[0]) - 3:
                # horizontal right
                if (
                    board[i][j] == board[i][j + 1] == board[i][j + 2] == turn
                    and board[i][j + 3] == 0
                ):
                    threes += 1
                if i < len(board) - 3:
            ...
            if i >= 3:
                # vertical
                if (
                    board[i][j] == board[i - 1][j] == board[i - 2][j] == turn
                    and board[i - 3][j] == 0
                ):
                    threes += 1
    return threes
...
def minimax(board, depth, alpha, beta, Maximizing, p, search_tree, pr):
    ...
    if Maximizing:
        ...
        for j in availabe_colomns:
            ...
            if pr == 1:
                stringg = (
                    f"Maximizing node, alpha: {alpha} beta: {beta} score: {maxscore}"
                )
            else:
            ...
            new_score = minimax(
                temp_board, depth - 1, alpha, beta, False, ran, search_tree, pr
            )[1]
    ...
    # Minimizing
        ...
        for j in availabe_colomns:
            ...
            if pr == 1:
                stringg = (
                    f"Minimizing node, alpha: {alpha} beta: {beta} score: {minscore}"
                )
            else:
            ...
            search_tree.create_node(stringg, ran, parent=p)
            new_score = minimax(
                temp_board, depth - 1, alpha, beta, True, ran, search_tree, pr
            )[1]
...
def expectiminimax(board, depth, alpha, beta, Maximizing, p, search_tree, pr):
    pruning = 0
    available_columns = get_playable_columns(board)
    terminal, winner = if_game_ended(board)

    # Check if we've reached the maximum depth or if the game has ended
    if depth == 0 or terminal:
        if terminal:
            if winner == COMPUTER:
                return (None, 1000000)
    ...
    if Maximizing:
        ...
        for j in available_columns:
            temp_board = drop_checker(board, j, COMPUTER)
            _,new_score = expectiminimax(
                temp_board, depth - 1, alpha, beta, False, p, search_tree, pr
            )[1]
            if new_score > maxscore:
                maxscore = new_score
                col = j
        ...
        if pr == 1:
            if pruning == 1:
                stringg = f"Maximizing node (pruning occurred), alpha: {alpha} beta: {beta} score: {maxscore}"
            else:
                stringg = f"Maximizing node (no pruning occurred), alpha: {alpha} beta: {beta} score: {maxscore}"
            search_tree.update_node(p, tag=stringg)
        return col, maxscore
    else:
        minscore = math.inf
        col = 0
        ...
        prob = [
            ...
            0.2,
            0.2,
        ]  # Probabilities for chosen column, left column, and right column respectively
        for j in available_columns:
            temp_board = drop_checker(board, j, PLAYER)
            _, new_score = expectiminimax(
                temp_board, depth - 1, alpha, beta, True, p, search_tree, pr
            )
            if new_score < minscore:
                minscore = new_score
                col = j
            ...
            if alpha >= beta and pr == 1:
                pruning = 1
                break
            total_prob += prob[j] * new_score
        expected_score = total_prob / len(available_columns)
...
expected_selected = (
    False  # Add a boolean variable to track if Expectiminimax button is selected
)
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if button_x <= mouse_pos[0] <= button_x + button_width:
                if minimax_button_y <= mouse_pos[1] <= minimax_button_y + button_height:
    ...
    if minimax_selected:
        pygame.draw.rect(
            screen, RED, (button_x, minimax_button_y, button_width, button_height), 3
        )
        prune = 0
    elif alphabeta_selected:
        pygame.draw.rect(
            screen, RED, (button_x, alphabeta_button_y, button_width, button_height), 3
        )
        prune = 1
    elif expected_selected:  
        pygame.draw.rect(
            screen,
            RED, (button_x, expectiminimax_button_y, button_width, button_height), 3
        )
        prune = 0
    pygame.display.update()

if minimax_selected or alphabeta_selected:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if get_playable_row(board, col) != None:
                        ...
                        search_tree.create_node("Parent", 0)
                        start_time = time.time()
                        nextcol, score = expectiminimax(
                            board, K, -math.inf, math.inf, True, 0, search_tree, prune
                        )
                        end_time = time.time()
                        runtime = end_time - start_time
                        print(f"Runtime of Expectiminimax = {runtime} seconds")
                        number_of_nodes = search_tree.size() - 1
                        print(f"Number of nodes: {number_of_nodes}")
                        stringg = f"Max score:{score}, Next colomn: {nextcol}"
                        search_tree.update_node(0, tag=stringg)
